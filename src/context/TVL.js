...
export const TVlValueContext = createContext();
const TVLContext = ({ children }) => {
    const {
        getPLSPrice,
        getPLSDepositors,
    ...
    const TotalVaultValueLocked = () => {
        ...
        const roundedTotal = Number(totalvalue.toFixed(15));
    ...
    return (
        <>
            <TVlValueContext.Provider
                value={{ totalVaultValue,setTotalVaultSum,TotalVaultValueLocked }}
            >
                {children}
            </TVlValueContext.Provider>
        </>
