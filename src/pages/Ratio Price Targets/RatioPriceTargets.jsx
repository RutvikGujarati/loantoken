...
export default function RatioPriceTargets() {
  ...
  const [PLSratioPriceTargets, setPLSRatioPriceTargets] = useState([]);
  const [price, setPrice] = useState("0");
  const [seeFullPage, setseeFullPage] = useState(false);
  const [nextPage, setNextPage] = useState(0);
  const [noOfPage, setNoOfPage] = useState(0);
  const [PLSnoOfPage, setPLSNoOfPage] = useState(0);
  ...
  const PLSRatioPriceTargets = async () => {
    if (accountAddress) {
      try {
        ...
        try {
          let items = await Promise.all(
            sortedArray.map((target, index) =>
              processPLSTargets(target, index, currencyName)
            )
          );
          setPLSRatioPriceTargets(items.filter(Boolean));
        } catch (error) {
          console.error("Error processing targets:", error);
        }
      } catch (error) {
  ...
  const processPLSTargets = async (target, index, currencyName) => {
    try {
      const formattedRatioTarget = ethers.utils.formatEther(
        target?.ratioPriceTarget.toString()
      );
