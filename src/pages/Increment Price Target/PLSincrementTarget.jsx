...
export default function PLSIncrementPriceTarget() {
  ...
  const { socket, getPLSPrice, getPLSIncrementPriceTargets, getPLSDepositors } =
  ...
  const processTargets = async (target, index, currencyName) => {
    try {
      ...
      const targetAmount =
        Number(formattedTargetAmount).toFixed(4) + " " + (await currencyName);
