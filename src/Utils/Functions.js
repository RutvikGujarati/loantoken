...
export default function Functions({ children }) {
	...
	const BalanceOfXenTokenContract = async (contractType = 'PSD') => {
		...
		try {
			...
			if (!contract) throw new Error('Contract instance is null or undefined.');

			const balance = await contract.balanceOf(address);
			return ethers.utils.formatEther(balance);
		} catch (error) {
			console.error(`Error fetching balance for ${contractType}:`, error.message);
			return '0';
		}
	};
	...
	const totalSupplyOfTokens = async (contractType = "DAV") => {
		let contract;

		try {
			// Get the appropriate contract based on the contract type
			switch (contractType) {
				case 'DAVDEFI':
			...
			if (!contract) throw new Error(`Contract instance for ${contractType} is null or undefined.`);

			// Ensure we await the totalSupply call
			const totalSupply = await contract.totalSupply();
			console.log(`Total Supply for ${contractType}:`, totalSupply);
			return totalSupply;
		} catch (error) {
