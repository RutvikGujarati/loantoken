 ...
 */
contract Autovaults_V1_2_linked_to_DAVDEFI is
    Ownable(msg.sender),
    ReentrancyGuard
{
    ...
    event AddressesUpdated(address indexed WATT, address indexed davpls);

    /**
     * @dev Initializes the contract with the specified token addresses and depositer address.
     * @param _WATTAddress The address of the XEN token contract.
     * @param _davDefiAddress The address of the davDefi token contract.
     * @param _depositerAddress The address of the depositer.
     */
    constructor(
        address _WATTAddress,
        address _davDefiAddress,
        address _depositerAddress
    ) {
        require(_WATTAddress != address(0), "Invalid XEN token address");
        require(_davDefiAddress != address(0), "Invalid davDefi token address");
        require(_depositerAddress != address(0), "Invalid depositer address");

        WATT = IERC20(_WATTAddress);
        davDefi = DAVDEFI(_davDefiAddress);
    ...
    /**
     * @dev Returns the addresses of the XEN and davDefi tokens.
     * @return PDXN The address of the XEN token.
     * @return DavPLS The address of the davDefi token.
     */
    function getAddresses() public view returns (address PDXN, address DavPLS) {
        return (address(WATT), address(davDefi));
    }
    ...
    /**
     * @dev Sets the addresses of the XEN and davDefi tokens.
     * @param PDXN The address of the XEN token.
     * @param davpls The address of the davDefi token.
     */
    function setAddresses(address PDXN, address davpls) public onlyOwner {
        require(PDXN != address(0), "Invalid XEN token address");
        require(davpls != address(0), "Invalid davDefi token address");

        WATT = IERC20(PDXN);
        davDefi = DAVDEFI(davpls);
    ...
    /**
     * @dev Distributes the auto vault fee to all davDefi token holders except the excluded user.
    ...
    function distributeAutoVaultFee(
        ...
        uint256 totalSupply = davDefi.totalSupply();
        uint256 excludeUserBalance = davDefi.balanceOf(excludeUser);
        ...
        uint256 holdersLength = davDefi.holdersLength();

        for (uint256 i = 0; i < holdersLength; i++) {
            address user = davDefi.holders(i);
            uint256 userBalance = davDefi.balanceOf(user);
    ...
    function deposit(uint256 value) public onlyDepositer {
        ...
        require(
            WATT.transferFrom(msg.sender, address(this), value),
    ...
    function contractTokenBalance() public view returns (uint256) {
        return WATT.balanceOf(address(this));
    }
    ...
    {
        uint256 length = davDefi.holdersLength();
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            balances[i] = davDefi.balanceOf(user);
        }
    ...
    /**
     * @dev Updates the parity amount for all davDefi token holders.
    ...
    function claimAllReward() public nonReentrant {
        ...
        require(
            WATT.transfer(user, allRewardAmount),
