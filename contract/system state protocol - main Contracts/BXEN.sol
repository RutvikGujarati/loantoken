 ...
 */
contract system_state_sc_Autovaults_V1_2_bFENIX is
    Ownable(msg.sender),
    ReentrancyGuard
{
    using SafeMath for uint256;

    IERC20 public bFENIX;
    ...
    event AddressesUpdated(address indexed bFENIX, address indexed davBnb);

    /**
     * @dev Initializes the contract with the specified token addresses and depositer address.
     * @param _bFENIXAddress The address of the XEN token contract.
     * @param _davBnbaAddress The address of the DavBnb token contract.
     * @param _depositerAddress The address of the depositer.
     */
    constructor(
        address _bFENIXAddress,
        address _davBnbaAddress,
        address _depositerAddress
    ) {
        require(_bFENIXAddress != address(0), "Invalid XEN token address");
        require(_davBnbaAddress != address(0), "Invalid DavBnb token address");
        require(_depositerAddress != address(0), "Invalid depositer address");

        bFENIX = IERC20(_bFENIXAddress);
        DavBnb = DAVBNB(_davBnbaAddress);
    ...
    function getAddresses() public view returns (address PDXN, address DavPLS) {
        return (address(bFENIX), address(DavBnb));
    }
    ...
    /**
     * @dev Sets the addresses of the XEN and DavBnb tokens.
     * @param PDXN The address of the XEN token.
     * @param davBnb The address of the DavBnb token.
     */
    function setAddresses(address PDXN, address davBnb) public onlyOwner {
        require(PDXN != address(0), "Invalid XEN token address");
        require(davBnb != address(0), "Invalid DavBnb token address");

        bFENIX = IERC20(PDXN);
        DavBnb = DAVBNB(davBnb);
        emit AddressesUpdated(PDXN, davBnb);
    }
    ...
    function deposit(uint256 value) public onlyDepositer {
        ...
        require(
            bFENIX.transferFrom(msg.sender, address(this), value),
    ...
    function contractTokenBalance() public view returns (uint256) {
        return bFENIX.balanceOf(address(this));
    }
    ...
    function claimAllReward() public nonReentrant {
        ...
        require(
            bFENIX.transfer(user, allRewardAmount),
