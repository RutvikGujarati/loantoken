...
import {DAVTRADE} from "../System state protocol-DAVToken contracts/DAVTRADE.sol";

/**
 * @title Autovaults V1.1
 * @dev A smart contract for managing deposits, distributing fees, and claiming rewards.
 */
contract Autovaults_V1_2_linked_to_DAVTRADE is
    Ownable(msg.sender),
    ReentrancyGuard
{
    using SafeMath for uint256;

    IERC20 public NINE_INCH;
    DAVTRADE public davTrade;
    uint256 public ID = 1;
    ...
    event AddressesUpdated(address indexed NINE_INCH, address indexed davpls);

    /**
     * @dev Initializes the contract with the specified token addresses and depositer address.
     * @param _NINE_INCHAddress The address of the XEN token contract.
     * @param _davTradeAddress The address of the davTrade token contract.
     * @param _depositerAddress The address of the depositer.
     */
    constructor(
        address _NINE_INCHAddress,
        address _davTradeAddress,
        address _depositerAddress
    ) {
        require(_NINE_INCHAddress != address(0), "Invalid XEN token address");
        require(_davTradeAddress != address(0), "Invalid davTrade token address");
        require(_depositerAddress != address(0), "Invalid depositer address");

        NINE_INCH = IERC20(_NINE_INCHAddress);
        davTrade = DAVTRADE(_davTradeAddress);
    ...
    /**
     * @dev Returns the addresses of the XEN and davTrade tokens.
     * @return PDXN The address of the XEN token.
     * @return DavPLS The address of the davTrade token.
     */
    function getAddresses() public view returns (address PDXN, address DavPLS) {
        return (address(NINE_INCH), address(davTrade));
    }
    ...
    /**
     * @dev Sets the addresses of the XEN and davTrade tokens.
     * @param PDXN The address of the XEN token.
     * @param davpls The address of the davTrade token.
     */
    function setAddresses(address PDXN, address davpls) public onlyOwner {
        require(PDXN != address(0), "Invalid XEN token address");
        require(davpls != address(0), "Invalid davTrade token address");

        NINE_INCH = IERC20(PDXN);
        davTrade = DAVTRADE(davpls);
    ...
    /**
     * @dev Distributes the auto vault fee to all davTrade token holders except the excluded user.
    ...
    function distributeAutoVaultFee(
        ...
        uint256 totalSupply = davTrade.totalSupply();
        uint256 excludeUserBalance = davTrade.balanceOf(excludeUser);
        ...
        uint256 holdersLength = davTrade.holdersLength();

        for (uint256 i = 0; i < holdersLength; i++) {
            address user = davTrade.holders(i);
            uint256 userBalance = davTrade.balanceOf(user);
    ...
    function deposit(uint256 value) public onlyDepositer {
        ...
        require(
            NINE_INCH.transferFrom(msg.sender, address(this), value),
    ...
    function contractTokenBalance() public view returns (uint256) {
        return NINE_INCH.balanceOf(address(this));
    }
    ...
    {
        uint256 length = davTrade.holdersLength();
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            balances[i] = davTrade.balanceOf(user);
        }
    ...
    /**
     * @dev Updates the parity amount for all davTrade token holders.
    ...
    function claimAllReward() public nonReentrant {
        ...
        require(
            NINE_INCH.transfer(user, allRewardAmount),
